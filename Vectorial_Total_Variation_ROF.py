...
u = noisy.copy()
p = np.zeros((u.shape[0], u.shape[1], 2, 3))
tau, sigma, weight = 0.125, 0.125, 0.15 # VTV 權重通常需稍大
...
for _ in range(200):
    ...
    # ★ 關鍵：axis=(2, 3) (同時計算空間與色彩通道的聯合模長)
    norm = np.sqrt(np.sum(p_new**2, axis=(2, 3), keepdims=True))
...
imsave('output_vectorial.png', img_as_ubyte(np.clip(u, 0, 1)))
